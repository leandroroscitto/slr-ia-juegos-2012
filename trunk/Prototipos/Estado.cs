   ...
   public class Estado {
	  ...
	  public bool IntentarAccion(Juego.Jugador jugador, Accion accion, out Vector2 nueva_posicion) {
		 if (accion.tipo == TAccion.MOVIMIENTO) {
			...
			if ((posicion_deseada.y < escenario_base[0].Length && posicion_deseada.x < escenario_base.Length) && (posicion_deseada.x >= 0 && posicion_deseada.y >= 0) && escenario_base[posicion_deseada.x][posicion_deseada.y].movilidad == Juego.Zona.TMovilidad.PASABLE) {
